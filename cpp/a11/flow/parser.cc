...
class ParserImpl {
 ...
 private:
  ...
  /// `skip n reference`, which removes the first `n` values for every reader.
  NodePtr ParseSkip(const Token& keyword) {
    auto skip = Make<syntax::Skip>(keyword);
    if (!At(TokenKind::kNumber)) {
      ReportHere("flow.syntax.unexpected",
                 absl::StrCat("Expected a positive count after 'skip', found ",
                              Found(),
                              ". Unused call outputs are drained "
                              "automatically."));
      auto pipeline = Make<syntax::Pipeline>(Current());
      pipeline->source = ParseReference();
      skip->targets.push_back(syntax::SkipTarget{
          .pipeline = std::move(pipeline), .call = {}, .outputs = {}});
    } else {
