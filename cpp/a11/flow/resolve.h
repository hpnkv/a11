...
///
/// The usage counts are here rather than in the inspector because the resolver is
/// the pass that walks every reference: counting as it goes costs nothing, and an
/// inspector that walked the tree again to find out would be a second
...
struct Symbol {
  ...
  bool writable = false;
  /// For a call: the action named, whether it was `try`, and the sibling flow it
  /// resolves to if it is one.
  std::string action;
  bool tolerant = false;
  ...
  /// node, a header, a loop variable. Memoised here because identity is the whole
  /// point -- two mentions of one node are one stream, and `skip n` accumulates
  /// on it.
...
struct ResolvedFlow {
  ...
  /// Every symbol the flow bound, in the order it bound them. Not a scope tree:
  /// scopes matter while resolving and what an inspector wants afterwards is the
  /// list, with the counts on it.
  std::vector<Symbol> symbols;
  /// The declaration this came from, borrowed. Valid as long as the parse result
  /// it belongs to is.
...
///
/// **What this is.** The semantic pass: it decides what every name in a flow means,
/// checks that a call's ports exist on the flow it names, that a type is a type,
...
/// the refs, steps and bodies the runtime walks -- beside the description. That is
/// off by default because an editor, `a11 flow check` and CI want none of it, and
/// a graph borrows the parse tree it was built from: whoever asks for one owns
/// both for as long as a flow is registered.
///
/// Like the parser, it **never throws and always returns**: a flow with an
/// unresolvable name in it is still resolved as far as it goes, because an editor
