...
/// object, so a flow that declares one should be able to hand it to anything that
/// speaks schemas -- a model's tool definition, an OpenAPI document, a validator
/// somewhere else -- and should be able to accept one back. Neither direction is
/// the "real" one: a shape is what the language reads and a schema is what the
/// world outside it reads, and this is the translation.
///
/// Draft 2020-12, because that is the draft JSON Schema settled on and the one a
/// model's structured-output mode expects. Every shape `struct` reaches, directly or
/// through another, goes into `$defs`, and a field naming one is a `$ref` -- so a
/// shape that names itself round-trips, which a schema inlined by substitution
/// could not.
///
/// **The three types JSON has no word for.** `bytes`, `time` and `duration` go
/// out as strings, with the encoding or format that says how to read them *and* a
/// `x-a11-type` beside it. The format alone is what another reader wants; the
/// extension is what makes coming back lossless, since `{"type": "string",
/// "format": "date-time"}` is also a perfectly good way to say `string`.
...
struct SchemaImport {
  ...
  ///
  /// In that order because the first is the one the caller asked about; the rest
  ...
  /// the shape it *did* describe plus a list of what was dropped, not nothing at
...
///
/// The reverse of [DtoToJsonSchema], and lossless for what that writes. Anything
...
///
/// What makes an import useful: the answer is source, so it can be pasted into a
/// file, read, edited and checked in. Written the way the formatter would write
/// it, so `a11 flow fmt` over the result changes nothing.
...
///
/// A JSON object's keys have no order a reader may rely on, and a shape's fields
