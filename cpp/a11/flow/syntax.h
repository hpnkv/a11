...
/// The one reference changed by a counted `skip`.
struct SkipTarget {
  PipelinePtr pipeline;
  Word call;
...
/// `skip n reference` -- discard the first `n` values for every reader.
///
/// `flow.syntax/v1` represents the target as a one-element vector.
