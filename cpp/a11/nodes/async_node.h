 ...
 * resolves once the backing store accepts the chunk. A stackless pump turn
 * enqueues each batch on attached `net::WireStream`s. The store confirmation
 * carries no mirror handover or remote-delivery acknowledgement.
 * WaitForBufferToDrain() covers both stages and reports tee failures.
 *
