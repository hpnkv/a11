...
/**
 * @brief Whether @p text is valid UTF-8, checked rather than caught.
 *
 * The reason this is a scan and not a `try`: nlohmann reports a string that
 * cannot be encoded as JSON by throwing, and in a translation unit compiled
 * `-fno-exceptions` -- which this library is -- its `JSON_THROW` becomes
 * `std::abort()`. a11::DumpJson does catch, and is compiled with exceptions on
 * for exactly that purpose, but `dump()` is a template: whichever instantiation
 * the linker keeps decides whether the failure is an exception or an abort, and
 * an aborting one from any other translation unit is a legal choice.
 *
 * It was not a legal outcome. `read_file` over a binary file killed the process
 * with no message, which is the worst way for a library to say "that is not
 * text". So the check happens here, before nlohmann is asked, where the answer
 * depends on the bytes and not on how something was compiled.
 */
bool IsValidUtf8(std::string_view text) {
  ...
  while (at < text.size()) {
    ...
    // all invalid, and all of them are things a filesystem will hand over.
...
/// The first string inside @p value that is not valid UTF-8, or nullptr.
///
/// Recursive because the strings that come from outside are not only at the top
/// level: a directory entry's `name` is a field of a record, and it is exactly
/// the field a filesystem can put arbitrary bytes in.
const nlohmann::json* absl_nullable FindUnencodableString(
  ...
  if (value.is_object() || value.is_array()) {
    for (const auto& element : value) {
      if (const nlohmann::json* found = FindUnencodableString(element);
          found != nullptr) {
        return found;
      }
    }
