...
std::vector<std::string> Coloured(std::string_view source,
  ...
  for (const SemanticToken& token : semantic) {
    ...
    out.push_back(
        absl::StrCat(SemanticKindName(token.kind), ":",
                     source.substr(token.start, token.end - token.start)));
  }
  return out;
}
...
TEST(FlowHighlight, ALevelIsALevelOnlyWhereALogTakesOne) {
  ...
  EXPECT_EQ(
      Coloured("a | logf error \"hm\""),
      (std::vector<std::string>{"identifier:a", "flow-operator:|", "stage:logf",
                                "log-level:error", "string:\"hm\""}));
  ...
  EXPECT_EQ(
      Coloured("log error.message"),
      (std::vector<std::string>{"statement-keyword:log", "identifier:error",
                                "punctuation:.", "member:message"}));
}

TEST(FlowHighlight, ADeclarationNamesItsFlow) {
  EXPECT_EQ(
      Coloured("flow research {}"),
      (std::vector<std::string>{"declaration-keyword:flow",
                                "flow-name:research", "brace:{", "brace:}"}));
}
...
TEST(FlowHighlight, APortTypeMayBeATagOrGenericAndStopsAtTheLine) {
  EXPECT_EQ(
      Coloured("out audio: a11.sdk.AudioBuffer stream"),
      (std::vector<std::string>{"declaration-keyword:out", "identifier:audio",
                                "punctuation::", "type:a11", "punctuation:.",
                                "type:sdk", "punctuation:.", "type:AudioBuffer",
                                "declaration-keyword:stream"}));
  EXPECT_EQ(Coloured("in frames: list[a11.NodeFragment] required"),
            (std::vector<std::string>{
                "declaration-keyword:in", "identifier:frames",
                "punctuation::", "type:list", "bracket:[", "type:a11",
                "punctuation:.", "type:NodeFragment", "bracket:]",
...
TEST(FlowHighlight, InOpensAPortDeclarationOnlyWhereANameAndAColonFollow) {
  EXPECT_EQ(
      Coloured("for hit in search.hits"),
      (std::vector<std::string>{"statement-keyword:for", "identifier:hit",
                                "declaration-keyword:in", "identifier:search",
                                "punctuation:.", "member:hits"}));
}

TEST(FlowHighlight, AWordIsAStageOnlyAfterAPipe) {
  EXPECT_EQ(
      Coloured("page.text | truncate 200 | count"),
      (std::vector<std::string>{
          "identifier:page", "punctuation:.", "member:text", "flow-operator:|",
          "stage:truncate", "number:200", "flow-operator:|", "stage:count"}));
...
TEST(FlowHighlight, ThenAndWhereAreStagesWithoutAPipeGivenAnOperand) {
  EXPECT_EQ(
      Coloured("hits where it.ok"),
      (std::vector<std::string>{"identifier:hits", "stage:where", "constant:it",
                                "punctuation:.", "member:ok"}));
...
TEST(FlowHighlight, ACallNamesAnActionAndItsModifiersAreModifiers) {
  EXPECT_EQ(
      Coloured(
          "page = try call web-fetch(url: hit.url) timeout 20s via fetched"),
      (std::vector<std::string>{
...
TEST(FlowHighlight, ABindingMayStillBeCalledRun) {
  ...
  EXPECT_EQ(Coloured("run = run some-action"),
            (std::vector<std::string>{"identifier:run",
                                      "operator:=", "statement-keyword:run",
...
TEST(FlowHighlight, AContextCarriesAcrossALineBreakAndAComment) {
  EXPECT_EQ(Coloured("call\n  some-action"),
            (std::vector<std::string>{"statement-keyword:call",
                                      "action-name:some-action"}));
  EXPECT_EQ(
      Coloured("call # which one\n  some-action"),
      (std::vector<std::string>{"statement-keyword:call", "comment:# which one",
                                "action-name:some-action"}));
}
...
TEST(FlowHighlight, OfJoinsAWaitsSubjects) {
  EXPECT_EQ(
      Coloured("wait first of a, b"),
      (std::vector<std::string>{"statement-keyword:wait", "identifier:first",
                                "statement-keyword:of", "identifier:a",
  ...
  EXPECT_EQ(Coloured("of = run act()"),
            (std::vector<std::string>{
                "identifier:of", "operator:=", "statement-keyword:run",
                "action-name:act", "parenthesis:(", "parenthesis:)"}));
}

TEST(FlowHighlight, AFunctionIsAFunctionWhereItIsCalled) {
  EXPECT_EQ(
      Coloured("| map join(it, \", \")"),
      (std::vector<std::string>{"flow-operator:|", "stage:map", "builtin:join",
                                "parenthesis:(", "constant:it", "punctuation:,",
                                "string:\", \"", "parenthesis:)"}));
...
TEST(FlowHighlight, AStatusCodeIsAConstantAndAFieldIsAField) {
  EXPECT_EQ(
      Coloured("fail not_found \"gone\""),
      (std::vector<std::string>{"statement-keyword:fail",
                                "status-code:not_found", "string:\"gone\""}));
...
TEST(FlowHighlight, ATagBeforeABraceIsAType) {
  EXPECT_EQ(
      Coloured("| map a11.sdk.Interaction{role: \"user\"}"),
      (std::vector<std::string>{
          "flow-operator:|", "stage:map", "type:a11", "punctuation:.",
          "type:sdk", "punctuation:.", "type:Interaction", "brace:{",
          "identifier:role", "punctuation::", "string:\"user\"", "brace:}"}));
...
TEST(FlowHighlight, ACastNamesAType) {
  EXPECT_EQ(Coloured("x as a11.sdk.AudioBuffer"),
            (std::vector<std::string>{"identifier:x", "declaration-keyword:as",
                                      "type:a11", "punctuation:.", "type:sdk",
...
TEST(FlowHighlight, SemanticKindNamesRoundTrip) {
  for (const SemanticKind kind : {SemanticKind::kComment,
...
TEST(FlowHighlight, ADeclarationEndsAtItsLine) {
  ...
  EXPECT_EQ(Coloured("x as a11.Chunk\nheader \"y\" as y"),
            (std::vector<std::string>{
                ...
                "punctuation:.", "type:Chunk", "declaration-keyword:header",
                "string:\"y\"", "declaration-keyword:as", "identifier:y"}));
}
...
TEST(FlowHighlight, APortLooksDifferentFromAFlowsOwnNames) {
  ...
  EXPECT_EQ(std::count(resolved.begin(), resolved.end(), "port-name:question"),
            2);
