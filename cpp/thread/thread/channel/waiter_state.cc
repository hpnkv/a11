...
static void Notify(CaseInSelectClause* absl_nonnull* absl_nonnull head,
                   CaseInSelectClause* absl_nonnull c)
...
void ChannelWaiterState::UnlockAndReleaseReader(
    CaseInSelectClause* absl_nonnull reader)
    ABSL_UNLOCK_FUNCTION(reader->selector->mu) {
  Notify(&readers, reader);
  reader->selector->mu.Unlock();
}

void ChannelWaiterState::UnlockAndReleaseWriter(
    CaseInSelectClause* absl_nonnull writer)
