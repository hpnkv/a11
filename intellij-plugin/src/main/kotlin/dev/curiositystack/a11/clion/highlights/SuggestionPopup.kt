 ...
 *
 * Sized like a tooltip rather than like a window: the diff inside is a handful
 * of lines and gets exactly the height those lines need, so this is the only
 * cap.
 */
...
/**
 * The popup on an analysed highlight: what A11 said about it, the fix it
 * proposed, a button that applies the fix, and the IDE's own message
 * underneath.
 *
...
internal object SuggestionPopup {
    ...
    /**
     * Where it was opened, so [refreshFor] can
     * put the replacement in the same place.
     */
    ...
    /**
     * Show the popup for [suggestion] in [editor], at [at] or at the range
     * itself.
     *
     * Returns null when the suggested range no longer exists after an edit.
     */
    fun showAt(project: Project, editor: Editor, suggestion: Suggestion, at: Point?): JBPopup? {
        val range = suggestion.range ?: return null
        hide()

        // Disposed with the popup: the diff panel holds editors, and an editor
        // that outlives the window showing it is a leak the platform asserts
        // on.
        ...
        val popup = JBPopupFactory.getInstance()
            ...
            .setResizable(true)
            .setMovable(true)
            // Not focused, because it opened on a hover: taking the caret away
            // from the editor because the mouse crossed a warning would be
            // hostile.
            .setRequestFocus(false)
            .setFocusable(true)
            .setCancelOnClickOutside(true)
            // No mouse-out cancel callback is installed on purpose: the user
            // has to be able to walk the pointer off the warning and into the
            // popup to press the button. Leaving the range is handled by
            // [SuggestionHover], which checks whether the mouse landed in here
            // first.
    ...
    /**
     * Redraw the popup for [suggestion], in place, if that is the one on
     * screen.
     *
     * The case this exists for: the user is hovering a range whose comment has
     * arrived and is reading it while the flow is still writing the patch for
     * it. When the patch lands the popup on screen is out of date by one diff
     * and an Apply button, and it would stay that way until the mouse left the
     * range and came back.
     *
     * Rebuilt rather than patched in place, because the content is assembled in
     * one pass from what the suggestion holds ([buildContent]) and rebuilding
     * it is what makes "the popup shows what has arrived" true without a second
     * code path. Reopened at exactly the point it was opened at, so the window
     * does not move under the mouse.
     */
    fun refreshFor(suggestion: Suggestion) {
        if (!isShowingFor(suggestion)) return
        // Read before showAt, which begins by calling hide() and clearing all
        // three.
     ...
     *
     * This is what stops the popup from closing as the user moves the mouse
     * off the warning and towards the Apply button: leaving the range is only
     * a reason to close if the pointer did not land here instead.
     */
    ...
    /**
     * `A11 · line 42`, so the popup says
     * which line it is about — and `· found` when
     * the range is one the model picked out rather than one the IDE reported,
     * since there is no warning here for the reader to attribute this to.
     */
    ...
    private fun buildContent(
        ...
        // One background for the whole popup, taken from the same place the
        // native tooltip label below takes its own:
        // `HintUtil.getInformationColor()`. The diff is painted on it too, so
        // the popup reads as one surface rather than as a tooltip with an
        // editor pasted into the middle of it.
        ...
        // Placed against the document as it stands now, not as it stood when
        // the model wrote the patch: "a valid patch" is one that still fits,
        // and it is this code — the same code that would apply it — that
        // decides.
        ...
        if (edits != null) {
            ...
            // Keep the explanation available even when its patch no longer
        ...
        nativeTooltip(project, editor, range)?.let { html ->
            ...
            // Preserve the platform tooltip's HTML and links in its native
     ...
     *
     * The compact, read-only editor follows the intention-preview layout. It
     * uses one column with source highlighting and theme diff colours. A full
     * `DiffRequestPanel` includes controls and a second editor that do not fit
     * the hover popup.
     */
    private fun diffView(
        ...
        // Renderer mode: no caret, no selection, nothing to interact with. This
        // is a picture of an edit, and clicking into it would only take the
        // focus.
        ...
        // The gutter is empty and would still paint a stripe of a different
        // colour down the left edge of the popup.
        pane.gutterComponentEx.isPaintBackground = false
        pane.gutterComponentEx.isVisible = false
        // The file's own highlighting, so the snippet reads as the language it
        // is.
        ...
        // Exactly as tall as the lines it holds: the popup's own scroll pane is
        // what deals with a patch too long for the screen, so there is no
        // second scrollable area for the wheel to fight over.
     ...
     *
     * `LINES_IN_RANGE`, so the colour runs
     * the full width of the popup rather than
     * stopping at the end of each line's text — a ragged right edge is what
     * makes a block of coloured lines read as highlighting instead of as a
     * diff. Taken from the scheme's own `DIFF_INSERTED`/`DIFF_DELETED`, so a
     * user who has themed their diffs sees those colours here too.
     */
    ...
    private fun applyRow(
        ...
        apply.addActionListener {
            // Located again, because the file may have changed while the popup
            // was open — including by the user applying the suggestion above
            // this one.
            ...
            // Applied is done with: the comment was about text that is no
            // longer there, and leaving the mark behind would invite applying
            // it twice.
